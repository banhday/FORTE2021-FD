...
Constraint2 ==
  ...
  /\ waitingTime >= 3 * Phi + Delta => existsMsgOfAge[Delta]
  
Constraint3 ==
  /\ (waitingTime >= 5 * Phi + Delta 
          => (rTimer >= waitingTime - 5 * Phi - Delta /\ rPC = "RNoSnd"))
  /\ ((waitingTime >= 4 * Phi + Delta /\ waitingTime < 5 * Phi + Delta) 
          => ( /\ (rPC = "RNoSnd" \/ rPC = "RComp")  
               /\ (rPC = "RComp" => rTimer >= waitingTime - 4 * Phi - Delta)))
  /\ ((waitingTime >= 3 * Phi + Delta /\ waitingTime < 4 * Phi + Delta) 
...
\* Last modified Wed Jan 20 08:11:56 CET 2021 by tran
\* Created Tue Jan 19 14:39:28 CET 2021 by tran
