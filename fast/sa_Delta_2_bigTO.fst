...
transition RRcv_NoDelivery_1 := {
    from := rnosnd;
    to := rrcv;
    guard := rTimer = 0 && rPC = 7 && buf < 4 && waitingtime < timeout;
    action := waitingtime' = waitingtime + 1;
};
...
Region init := { state = rcomp && sTimer = 0 && rTimer = 0 
    ...
    && waitingtime = 0 && timeout = phi + phi + phi + phi + phi + phi + 2 };    
