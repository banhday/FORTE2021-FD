...
CONSTANTS N,              \* The number of processes  
          ...
          Phi,            \* Bound for relaive process speed
                          \* Move the definition of Below constants are APALACHE
          Proc,           \* Proc == 1..N  
          MsgAge,         \* MsgAge == 0..Delta  
          TOGuard,        \* TOGuard == 3 * Phi + Delta + 2  
          TODefault,      \* TODefault == 1 \/ TODefault == TOGuard
          TOSet,          \* TOSet == 0..TOGuard
          Break_Period,   \* Break_Period == 0..(Phi - 1)
          MsgAgePowerSet, \* MsgAgePowerSet == [Proc \times Proc -> SUBSET (0..Delta)]
          FromLastSendSet \* FromLastSendSet == 0..(3 * Phi - 1)
...
\*  - Initial states 
\*  - Initial states might not satisfy the inductive invariant BigIndInvInPostfix.
\*  However, for every execution path, there exists a postfix such that every 
\*  state in this postfix satisfies BigIndInvInPostfix. This property is checked 
...
\* If pc[p] = "crash" or p = q, then 0. Otherwise, TOGuard.
TOVal == 
  time_out = [ <<p, q>> \in Proc \times Proc |-> 
                  IF pc[p] = "crash" \/ p = q
                  THEN 0
...
MsgBufVal ==
  ...
  /\ \A rcver, snder \in Proc :                                         
          IF pc[rcver] = "crash" \/ pc[snder] = "crash" \/ snder = rcver 
          THEN msgBuf[snder, rcver] = {} <: {Int}
...
DelVal ==              
  /\ del \in [Proc \times Proc -> BOOLEAN]                                
  /\ \A snder, rcver \in Proc :                                         
        ((pc[snder] = "crash" \/ pc[rcver] = "crash" \/ snder = rcver)
            => ~del[rcver, snder])  
...
\* turn. In other words, when phase = "sched".     
BigIndInvInPostfix ==
  \/ phase \in {"send", "del", "comp"} 
  \/ /\ phase = "sched"
     /\ PostfixConstraints
...
Del_IndInv ==
  ...
  /\ \A rcver \in Proc : 
        /\ ~del[rcver, rcver]
        /\ pc[rcver] = "crasz" => \A snder \in Proc : ~del[rcver, snder]
                                    
Phase_IndInv == phase \in {"sched", "send", "del", "comp"}                                
...
\* Whilte we can write Init_BigIndInv == phase = "sched" /\ BigIndInv, APALACHE
...
Spec_BigIndInv == Init_BigIndInv /\ [][Next]_vars /\ WF_vars(Next)
...
\* Last modified Thu Dec 05 15:02:24 CET 2019 by tthai
